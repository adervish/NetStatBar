    ...
    func exportTSV(limit: Int? = nil) -> String {
        ...
        dbQueue.sync {
            ...
            while sqlite3_step(stmt) == SQLITE_ROW {
                ...
                let date    = fmt.string(from: Date(timeIntervalSince1970: ts))
                rows.append("\(date)\t\(latency)\t\(col(stmt,2))\t\(col(stmt,3))\t\(sqlite3_column_int(stmt,4))\t\(sqlite3_column_int(stmt,5))\t\(col(stmt,6))\t\(col(stmt,7))\t\(col(stmt,8))\t\(col(stmt,9))")
            }
        }
